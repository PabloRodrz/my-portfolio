  ...
  return (
    <Container fluid ref={ref} className='skills d-flex flex-column gap-1'>
      <Row className={`d-flex justify-content-center ${isDesktop < 992 ? 'mt-5' : 'mt-2'}`}>
        <h1 style={{ fontWeight: '700' }} className={`text-white text-center ${isDesktop < 992 ? 'mt-5' : 'mt-2'}`}>Skills</h1>
      ...
      <Container fluid className='d-flex flex-column align-items-center justify-content-center mt-5'>
        <Row className='d-flex'>
          ...
          {/* {
            Object.keys(LOGOS_WITH_NAMES_FIRST_ROW).map((image, index) => (
              <Col key={index} className='d-flex flex-row'>
                <Card image={PROJECTS[image]} />
              </Col>
            ))
          } */}
          {/* <Col className='d-flex align-content-center justify-content-center gap-5'>
            {Object.keys(LOGOS_WITH_NAMES_FIRST_ROW).map((item) => (
              <OverlayTrigger
                key={item}
                placement="top"
